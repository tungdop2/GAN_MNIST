...
class MNISTGAN(pl.LightningModule):
    def __init__(self, cfg):
        ...
        self.vid = cv2.VideoWriter('out.avi', cv2.VideoWriter_fourcc(
            'M', 'J', 'P', 'G'), 10, (122, 122))
    ...
    def training_step(self, batch, batch_idx, optimizer_idx):
        real_imgs, _ = batch
        noise = torch.randn(real_imgs.size(
            0), self.cfg.noise_size, 1, 1).to(device)
    ...
    def configure_optimizers(self):
        ...
        scheduler_G = torch.optim.lr_scheduler.StepLR(
            optimizer_G, step_size=50, gamma=0.95)
        scheduler_D = torch.optim.lr_scheduler.StepLR(
            optimizer_D, step_size=50, gamma=0.95)
    ...
    def on_epoch_end(self):
        ...
        grids = torchvision.utils.make_grid(
            fake_imgs, nrow=4, padding=2, normalize=True)
        if self.current_epoch % cfg.save_interval == 0:
            self.logger.experiment.add_image(
                'fake_images', grids, self.current_epoch)

        frame = grids.mul(255).add_(0.5).clamp_(0, 255).permute(
...
trainer = pl.Trainer(gpus=num_gpus, max_epochs=cfg.num_epochs,
                     logger=logger, checkpoint_callback=False)
